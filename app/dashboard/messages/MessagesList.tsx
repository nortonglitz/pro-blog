  ...
  const handleMessageClick = (id: number, message: Message) => {
    try {
      ...
      setMessages(prevMessages =>
        prevMessages.map(prevMsg => (prevMsg.id === id ? { ...prevMsg, read: true } : prevMsg))
      )
    } catch {
    ...
    <>
      ...
      <div className="w-full flex flex-col">
        {messages.map((message, i) => (
          ...
          >
            <span>{message.read ? <IconMailOpened stroke={1.5} /> : <IconMail stroke={1} />}</span>
            <div className="flex flex-wrap flex-1 justify-between">
              <span className="line-clamp-1 text-left font-semibold flex-1 min-w-60">
                {message.subject}
              </span>
              <span className="text-sm md:text-base text-left flex-1 min-w-60">{message.from}</span>
            </div>
